...
const saveConfig = (cookieName, configObject) => {
    ...
    cookies.set(`${COOKIE_PREFIX}_${cookieName}`, JSON.stringify(configObject), {maxAge: 315360000})
}

const readConfig = (cookieName) => {
    const cookies = new Cookies();
    const rawCookie = cookies.get(`${COOKIE_PREFIX}_${cookieName}`, {doNotParse: true});
    if (!rawCookie) return null;
    const configObject = JSON.parse(rawCookie);
    return configObject
}

export const setAndSaveStateWrapper = (newStateFunc, currentState, setStateFunc, saveCookieFunc) => {
...
export const readDiamondIngredientsConfig = () => {
    return readConfig('diamonds_ingredient_config');
}
...
export const readCostLimits = () => {
    return readConfig('cost_limit');
}
...
export const readDiplomas = () => {
    return readConfig('diplomas');
}
...
export const readDesiredEffects = () => {
    return readConfig('desired_effects');
}
...
export const readPotion = () => {
    return readConfig('potion');
}
